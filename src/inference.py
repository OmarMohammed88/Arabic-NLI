...
parser.add_argument('--gpu',type = str, help='directory of checkpoints')
...
def inference(sentence1,sentence2, model, tokenizer, device):
    # tokenize the text
    encoded_text = tokenizer(sentence1, sentence2, truncation=True, padding="max_length",return_tensors='pt').to(device)
    ...
    dec = {1:'Entails',1:'No Entails',2:'neutral'}
    return dec[label]
